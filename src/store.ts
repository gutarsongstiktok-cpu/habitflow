...
export const useStore = create<Store>((set,get) => ({
  ...
  hydrate: async () => {
    try {
      const data = await Promise.race([loadCloud(), new Promise<null>(r => setTimeout(()=>r(null),1400))]);
      const local = loadLocal();
      set({ ...(data ?? local ?? initial), hydrated:true });
    } catch {
      set({ ...(loadLocal() ?? initial), hydrated:true });
    }
  },
