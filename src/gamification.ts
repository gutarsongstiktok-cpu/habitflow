...
export const DEFAULT_GAMIFICATION: GamificationData = { xp: 0, coins: 0, achievements: [], events: [] };
...
export const ACHIEVEMENTS = [
  ...
  { id: 'perfect-day', icon: '🏆', title: 'Идеальный день', desc: 'Выполнить все привычки за день', xp: 100 },
...
export function levelProgress(xp: number) {
  const level = levelFromXp(xp), start = levelStart(level), end = levelEnd(level);
  return { level, current: Math.max(0, xp - start), needed: end - start, pct: Math.min(100, Math.round(((xp - start) / (end - start)) * 100)) };
}
